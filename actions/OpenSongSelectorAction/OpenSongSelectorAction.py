...
class OpenSongSelectorAction(PlaybackAction):
    ...
    def generate_pages(self, songs):
        ...
        for p_idx, page_songs in enumerate(pages):
            ...
            def make_state(action_id, action_settings, label_text):
                return {
                    "states": {
                        "0": {
            ...
            page_data["keys"]["0x0"] = make_state("com_midiplusplus_Controller::CloseSongSelector", {}, "Cancel")
            
            # Next button
            if p_idx < len(pages) - 1:
                next_x = cols - 1
                next_y = rows - 1
                page_data["keys"][f"{next_x}x{next_y}"] = make_state("com_midiplusplus_Controller::LoadGeneratedPage", {"page_idx": p_idx + 1}, "Next ->")
            
            # Previous button
            if p_idx > 0:
                prev_x = 0
                prev_y = rows - 1
                page_data["keys"][f"{prev_x}x{prev_y}"] = make_state("com_midiplusplus_Controller::LoadGeneratedPage", {"page_idx": p_idx - 1}, "<- Prev")
            ...
            for y in range(rows):
                for x in range(cols):
                    ...
                    if song_idx < len(page_songs):
                        ...
                        page_data["keys"][f"{x}x{y}"] = make_state("com_midiplusplus_Controller::LoadSong", {"song_index": global_idx}, display_name)
                        song_idx += 1
